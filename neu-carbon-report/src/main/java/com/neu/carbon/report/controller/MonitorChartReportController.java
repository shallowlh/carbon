...
public class MonitorChartReportController extends BaseReportController {
    ...
    @DynamicResponseParameters(name = "carbonReportEmissionChartOverall", properties = {@DynamicParameter(name = "total", value = "总记录数"),
...
// 三年前第一天数据
        Date threeYear = DateUtil.offset(new Date(), DateField.YEAR, -2);
        Date first = DateUtil.beginOfYear(threeYear);
...
// 构造图表数据结构
    ...
    @DynamicResponseParameters(name = "carbonReportEmissionChartPercent", properties = {@DynamicParameter(name = "total", value = "总记录数"),
            @DynamicParameter(name = "code", value = "状态码，200正确，其他错误"),
            @DynamicParameter(name = "data", value = "返回业务数据（Map类型）", dataTypeClass = VMesProductPowerDetail.class),
            @DynamicParameter(name = "msg", value = "返回消息内容")})
