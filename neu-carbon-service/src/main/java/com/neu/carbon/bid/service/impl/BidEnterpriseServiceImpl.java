...
public class BidEnterpriseServiceImpl implements IBidEnterpriseService {
    ...
    @Override
    public BidEnterprise selectBidEnterpriseByUserName(String userName) {
        return bidEnterpriseMapper.selectBidEnterpriseByUserName(userName);
    }
    ...
    public int insertBidEnterprise(BidEnterprise bidEnterprise) {
        bidEnterprise.setPass(false);
        bidEnterprise.setCreateTime(DateUtils.getNowDate());
        // 此处采用 User 与 Enterprise 一对一关联，弃用基于 Enterprise 创建 User，要求必须用已有用户创建企业，并保证企业与用户一对一
...
//        userService.insertUser(user);
     ...
     */
    @Override
    public BidEnterprise getBidEnterpriseByUserName(String username) {
        return bidEnterpriseMapper.getBidEnterpriseByUserName( username);
    }
}
